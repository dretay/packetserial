...
void test_build_packet(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8_ARRAY(&data, &packet.data.bytes, 4);
...
uint8_t dummy_test_send_big_packet_messages[3][100];
...
bool dummy_test_send_big_packet_packet_parser(pb_istream_t* stream)
{
    ...
    if (ProtoBuff.decode(stream, Packet_fields, &packet)) {
        ...
        memcpy(dummy_test_send_big_packet_messages[dummy_test_send_big_packet_counter], &packet.data.bytes, packet_data_size);
        ...
        if (packet.flag == Packet_Flag_LAST) {
            ...
            TEST_ASSERT_EQUAL_STRING("test7test8", dummy_test_send_big_packet_messages[2]);
        }
    } else {
        TEST_FAIL();
        return false;
...
{
    uint8_t decoded_data_array[size];
    memset(decoded_data_array,0,size);
    cobs_decode(decoded_data_array, BUFFER_SIZE, buffer, size - 1);
...
void test_send_big_packet(void)
{
    ...
    uint8_t data[] = "test1test2test3test4test5test6test7test8";
    bool success = PacketSerial.send(data, sizeof(data));
    TEST_ASSERT_TRUE(success);
}
...
bool dummy_test_send_big_hid_report_packet_parser(pb_istream_t* stream)
{
    Packet packet;
    if (ProtoBuff.decode(stream, Packet_fields, &packet)) {
        int packet_data_size = strlen(packet.data.bytes);
        log_trace("received a packet with data of size %d",packet_data_size);
        memcpy(dummy_test_send_big_hid_report_messages_offset, &packet.data.bytes, packet_data_size);
        dummy_test_send_big_hid_report_messages_offset += packet_data_size;

        if (packet.flag == Packet_Flag_LAST) {
            char hex_str[300];
            string2hexstring((char*)dummy_test_send_big_hid_report_messages, hex_str);
            log_trace("whole received message: %s", hex_str);
...
bool dummy_test_send_big_hid_report_tx_handler(u8* buffer, size_t size)
{
    ...
    string2hexstring((char*)decoded_data_array, hex_str);
    log_trace("\t (decoded cobs) %s", hex_str);
...
void test_send_big_hid_report(void)
{
    ...
    char hex_str[(size * 2) + 1];
    memset(hex_str,0,(size * 2) + 1);
    string2hexstring((char*)my_buffer, hex_str);
    log_trace("whole sent message: %s", hex_str);
